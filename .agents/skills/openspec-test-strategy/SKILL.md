...
description: Поведенческая TDD-стратегия тестирования для OpenSpec/SDD в Дэшборд Нефтегаз. Используй при создании или обновлении OpenSpec-артефактов (`proposal.md`, `design.md`, `tasks.md`, specs, `test-plan.md`), выборе unit/component/E2E уровней для frontend, проверке правил frontend-тестов, покрытия сценариев и рисков, verification gates, Definition of Ready и Definition of Done.
---
...
- Применяй лейблы Allure ко всем frontend-тестам: Unit-front, Component-front или E2E-front layer, Feature по домену. Для unit/component используй setup files `src/test/setup-allure-unit.ts` и `src/test/setup-allure-component.ts`; для E2E импортируй `test`/`expect` из `apps/frontend/e2e/fixtures.ts`, а не напрямую из `@playwright/test`.
...
- [ ] backend: npm run lint / npm test / npm run test:e2e, если затронут
...
- Backend: из `apps/backend` запускай `npm test`, `npm run lint`, `npm run test:e2e`.
