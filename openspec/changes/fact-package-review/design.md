...
- владелец — либо Филиал (`filialId`, идёт на проверку выбранным ЦФО), либо
  ЦФО (`cfoId`, свой пакет без проверки ЦФО, сразу в ДТОиР) — ровно одно из
  двух полей заполнено (nullable FK), без DB-уникальности пары «владелец ×
  направление»: каждое обращение к действию «Создать» на экране «Файлы»
  создаёт новый факт-пакет с новым человекочитаемым ID (по аналогии с
  `Correction.create()`), владелец может иметь несколько одновременно;
...
### Создание факт-пакета (`POST /fact-packages`)

Изначально (см. history) был сделан идемпотентный get-or-create
(`GET /fact-packages/by-direction/{direction}`) под допущение «один
долгоживущий пакет на пару Филиал×Направление» — по результатам ревью и
дальнейшего уточнения продукта это допущение отменено: экран «Файлы» должен
работать как «Создать корректировку» (явное действие, всегда новая сущность).
`POST /fact-packages` (роли FILIAL и CFO, body `{direction}`) создаёт новый
факт-пакет в транзакции в статусе `DRAFT` вместе со всеми формами каталога
направления (без версий), возвращает `FactPackageDetail`; владелец
определяется ролью вызывающего — `filialId = user.filialId` для FILIAL,
`cfoId = user.cfoId` для CFO. Старый `GET .../by-direction/{direction}`
удалён из контракта.

### Факт-пакет, созданный ЦФО

ЦФО может быть не только проверяющим (через `FactPackageCfoStatus`), но и
владельцем своего факт-пакета (`cfoId` заполнен, `filialId` NULL) — тот же
каталог форм и та же механика загрузки версий, что у Филиала, но без шага
проверки ЦФО: `submit()` для такого пакета не принимает `cfoIds` и переводит
статус сразу в `UNDER_DTOE_REVIEW` (минуя `UNDER_CFO_REVIEW`/
`ALL_CFO_APPROVED`), не создавая строк `FactPackageCfoStatus`. Владение
проверяется отдельным guard'ом `isPackageOwner` (Филиал по `filialId`, ЦФО по
`cfoId`) — переиспользуется в `uploadFormVersion`/`submit`/`fixRemark`.
`filialId`/`cfoId` на `FactPackage` — оба nullable FK, ровно один заполнен;
`filial`/`cfo` в DTO — nullable, `FactPackageDetail.isCfoOwner` дополняет
`isFilialOwner` на фронте для общих guard'ов (`canUploadFormVersion` и т.п.).
