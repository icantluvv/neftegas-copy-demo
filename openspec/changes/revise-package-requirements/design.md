...
**Реализовано**: разделитель — не отдельная строка таблицы (усложнило бы
`ColumnDef`/`DataTable` синтетическими строками), а надпись `groupLabel —
выберите один вариант` (`text-xs font-semibold text-muted-foreground`) над
`label` первой строки группы, внутри той же ячейки колонки «Элемент»
(`SlotLabelCell` в `package-completeness.tsx`). Визуально эквивалентно
описанному выше решению по умолчанию, технически проще при существующей
табличной архитектуре. Вопрос более выраженного груп-блока (рамка, фон,
аккордеон) остаётся открытым при отсутствии дизайн-указаний пользователя —
если поступят, фиксируются здесь немедленно.
...
`ready with conditions` — backend, API-схема и frontend реализованы и
проверены `curl`; условие — визуальное решение группы в «Дизайн (UI)»
временное (текстовая надпись вместо выраженного груп-блока), может быть
скорректировано позже без блокировки; component/E2E-тесты и визуальная
проверка в браузере — waiver до появления Chromium в dev-контейнере
фронтенда.
