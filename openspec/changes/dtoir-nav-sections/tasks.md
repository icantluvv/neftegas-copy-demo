...
- [x] `bun run test` (apps/frontend) — 159/159, включая component-тесты.
      Ранее не выполнялись из-за отсутствующих системных библиотек Chromium в
      dev-контейнере; устранено через `bunx playwright install-deps chromium`.
      Дополнительно найдено и исправлено: `top-tabs`/`sidebar-nav`
      component-тесты мокали `usePathname` через `vi.mock('next/navigation')`,
      что не работает поверх глобального Vite-алиаса на `src/test/mocks/
      next-navigation.ts` — тесты «зелёными» проходили только потому, что
      Chromium не запускался и весь файл падал целиком. Переписаны на
      мутацию `nextNavigationMock.pathname` по конвенции проекта.
