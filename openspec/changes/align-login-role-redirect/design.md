...
проверке формата; иначе демо-вход перестанет проходить валидацию.

**Уточнение по ходу реализации**: `@IsEmail()` на `LoginDto` фактически не
исполняется для `POST /auth/login` — `LocalAuthGuard` (Guards-фаза) вызывает
`LocalStrategy.validate()` раньше, чем `ValidationPipe` успевает провалидировать
`@Body() LoginDto` (Pipes-фаза выполняется после Guards в жизненном цикле
запроса Nest). Поэтому проверка формата email перенесена непосредственно в
`LocalStrategy.validate()` (`isEmail()` из `class-validator`,
`BadRequestException` при несовпадении) — `@IsEmail()` в `LoginDto` оставлен
для документации контракта через `@nestjs/swagger`, но не является
