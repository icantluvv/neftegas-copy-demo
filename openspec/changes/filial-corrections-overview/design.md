...
- **Добавлено в ЧТЗ после первой версии этого change**: раздел 7.3 описывает
  новый элемент шапки «Переключатель роли» (иконка-стрелка рядом с названием
  филиала, поповер с селектом ролей/филиалов, список — запросом к бэку). Сам
  ЧТЗ помечает его открытым вопросом (раздел 8, пункт 6): переключает ли
  селект доступные пользователю филиалы или функциональные роли
  (Филиал/ЦФО/ДТОиР) — от ответа зависит, что происходит при выборе (смена
  фильтра на месте vs переход `/filial` → `/cfo`), и ЧТЗ прямо требует
  «уточнить у заказчика перед реализацией». Сознательно НЕ реализуется этим
  change по той же причине, по которой не реализован `/corrections/[id]/edit`
  (раздел 0.3 ЧТЗ) — implementation, опережающая нерешённый вопрос заказчика,
  рискует потребовать переделки при любом из двух ответов. Текущий сайдбар
  (`sidebar-nav.tsx`) элемент не содержит; шапка приватного layout
  (`(private)/layout.tsx`) содержит только `NotificationBell`.
