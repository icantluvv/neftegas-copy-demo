...
import { useSendCorrection } from "@/packages/api/base/codegen";

import { Button } from "#/components/ui/button";

import { canSendForReview } from "../lib/permissions";

export function SendForReviewForm({ detail }: { detail: CorrectionDetail }) {
  const queryClient = useQueryClient();
  const [selectedCfoIds, setSelectedCfoIds] = useState<number[]>([]);
  const sendCorrection = useSendCorrection({
    mutation: {
      onSuccess: () => {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border p-4">
      ...
      <Button
        type="button"
        title={hint}
        disabled={!sendAllowed || sendCorrection.isPending}
        onClick={() => sendCorrection.mutate({ humanId: detail.humanId, data: { cfoIds: selectedCfoIds } })}
      >
        Направить
      </Button>
    </div>
