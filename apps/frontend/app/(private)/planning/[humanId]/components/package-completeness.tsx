...
import type { PlanDetail, PlanDocumentSlot2, PlanRemarkCreateInput } from "@/packages/api/base/codegen";
import { getPlanSuspenseQueryKey } from "@/packages/api/base/codegen/hooks/plansController/useGetPlanSuspense";
import { useLeavePlanRemark, useUploadPlanFileVersion } from "@/packages/api/base/codegen";
...
function openRemarkForSlot(detail: PlanDetail, slotId: number) {
  return detail.remarks.find((remark) => remark.relatedSlotId === slotId && remark.status !== "CLOSED");
}

function SlotLabelCell({ slot, canDownload }: { slot: PlanDocumentSlot2; canDownload: boolean }) {
  ...
  return (
    <a
      href={`${clientEnvironment.NEXT_PUBLIC_BACK_URL}/plan-files/${slot.currentVersion.id}/download`}
      target="_blank"
...
function CurrentVersionCell({ slot, canDownload }: { slot: PlanDocumentSlot2; canDownload: boolean }) {
...
function UploadSlotFileCell({ detail, slot }: { detail: PlanDetail; slot: PlanDocumentSlot2 }) {
  const queryClient = useQueryClient();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const upload = useUploadPlanFileVersion({
    mutation: {
      onSuccess: () => {
...
function LeaveRemarkCell({ detail, slot }: { detail: PlanDetail; slot: PlanDocumentSlot2 }) {
  const queryClient = useQueryClient();
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: getPlanSuspenseQueryKey({ humanId: detail.humanId }) });

  const leaveRemark = useLeavePlanRemark({ mutation: { onSuccess: () => void invalidate() } });

  if (!canLeaveRemarkAsCfo(detail) && !canLeaveRemarkAsDtoe(detail)) {
    return null;
  }

  function handleSubmit(data: PlanRemarkCreateInput) {
...
export function PackageCompleteness({ detail }: { detail: PlanDetail }) {
  ...
  const columns: ColumnDef<PlanDocumentSlot2, unknown>[] = [
    ...(isReviewer
      ? [
          {
            id: "reviewed",
            header: "Проверено",
            cell: ({ row }: { row: { original: PlanDocumentSlot2 } }) => (
              <ReviewedCheckbox
          ...
          } satisfies ColumnDef<PlanDocumentSlot2, unknown>,
        ]
      : []),
    {
      accessorKey: "label",
