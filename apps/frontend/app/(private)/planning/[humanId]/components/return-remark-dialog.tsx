...
import type { PlanRemarkCreateInput } from "@/packages/api/base/codegen";
import { planRemarkCreateInputSchema } from "@/packages/api/base/codegen";
...
interface ReturnRemarkDialogProps {
  ...
  onSubmit: (data: PlanRemarkCreateInput) => void;
...
export function ReturnRemarkDialog({ triggerLabel, isSubmitting, onSubmit, slotId, slotLabel }: ReturnRemarkDialogProps) {
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<PlanRemarkCreateInput>({
    resolver: zodResolver(planRemarkCreateInputSchema),
  });

  function handleFormSubmit(data: PlanRemarkCreateInput) {
