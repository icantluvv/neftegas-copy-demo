...
import {
  CfoStatusValue,
  CorrectionCfoStatus,
} from './entities/correction-cfo-status.entity';
...
export class CorrectionsService {
  constructor(
    ...
    private fileVersions: Repository<FileVersion>,
    @InjectRepository(CorrectionCfoStatus)
    private cfoStatuses: Repository<CorrectionCfoStatus>,
    @InjectRepository(Remark) private remarks: Repository<Remark>,
    @InjectRepository(CorrectionHistoryEntry)
    private history: Repository<CorrectionHistoryEntry>,
  ...
  async findAll(user: User, query: FindCorrectionsQueryDto) {
    ...
    if (query.filialId)
      qb.andWhere('c.filialId = :filialId2', { filialId2: query.filialId });
    if (query.correctionTypeId)
      qb.andWhere('c.correctionTypeId = :typeId', {
        typeId: query.correctionTypeId,
      });
    ...
    if (user.role === Role.CFO && items.length) {
      const rows = await this.cfoStatuses.find({
        where: {
          correctionId: In(items.map((i) => i.id)),
          cfoId: user.cfoId ?? -1,
        },
      });
      myStatusesByCorrection = new Map(
        rows.map((r) => [r.correctionId, r.status]),
      );
    }

    return {
      items: items.map((c) =>
        toCorrectionListItemDto(c, {
          myCfoStatus: myStatusesByCorrection.get(c.id) ?? null,
        }),
      ),
      total,
      page,
  ...
  async getStats(user: User) {
    if (user.role === Role.CFO) {
      ...
      return {
        total: rows.length,
        inReview: rows.filter((r) => r.status === CfoStatusValue.PENDING)
          .length,
        returned: rows.filter((r) => r.status === CfoStatusValue.RETURNED)
          .length,
        approved: rows.filter((r) => r.status === CfoStatusValue.APPROVED)
          .length,
      };
    }
    ...
    const base = {
      ...
      approved: all.filter(
        (c) => c.status === CorrectionStatus.APPROVED_BY_DTOE,
      ).length,
    };
    ...
    if (user.role === Role.DTOE || user.role === Role.ADMIN) {
      const openRemarks = await this.remarks.count({
        where: {
          status: In([
            RemarkStatus.OPEN,
            RemarkStatus.FIXED_BY_FILIAL,
            RemarkStatus.REOPENED,
          ]),
        },
      });
      return {
        ...
        underReview: all.filter(
          (c) => c.status === CorrectionStatus.UNDER_DTOE_REVIEW,
        ).length,
        openRemarks,
      };
    }
  ...
  async create(user: User, dto: CreateCorrectionDto) {
    ...
    if (!correctionType)
      throw new NotFoundException('Тип корректировки не найден');
  ...
  async downloadFileVersion(user: User, id: number) {
    const version = await this.fileVersions.findOne({
      where: { id },
      relations: ['slot', 'slot.correction'],
    });
    if (!version) throw new NotFoundException();
    const correction = await this.findByHumanIdOrThrow(
      version.slot.correction.humanId,
    );
    if (!(await this.checkAccess(user, correction))) {
      throw new ForbiddenException();
    }
  ...
  async cfoApprove(user: User, humanId: string) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      const toClose = openRemarks.filter(
        (r) => r.status !== RemarkStatus.CLOSED,
      );
    ...
    await this.dataSource.transaction(async (manager) => {
      for (const cfoId of dto.cfoIds) {
        const status = await manager
          ...
          .findOne({ where: { correctionId: correction.id, cfoId } });
        if (!status) continue;
        await manager.getRepository(CorrectionCfoStatus).update(status.id, {
          status: CfoStatusValue.PENDING,
          decidedById: null,
          decidedAt: null,
        });
      ...
      await manager
        ...
        .update(correction.id, { status: CorrectionStatus.RESUBMITTED });
      await this.log(
        manager,
        correction.id,
        user,
  ...
  async reopenRemark(
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      const status = await manager.getRepository(CorrectionCfoStatus).findOne({
  ...
  async markRemarkFixed(user: User, humanId: string, remarkId: number) {
    ...
    await this.dataSource.transaction(async (manager) => {
      await manager
        .getRepository(Remark)
        .update(remark.id, { status: RemarkStatus.FIXED_BY_FILIAL });
      await this.log(
        manager,
        correction.id,
        user,
  ...
  async dtoeApprove(user: User, humanId: string) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.log(
        ...
        correction.id,
        user,
        'ДТОиР согласовало корректировку. Финальный статус: Согласовано ДТОиР.',
      );
    });
  ...
  async resubmitToDtoe(user: User, humanId: string) {
    ...
    await this.dataSource.transaction(async (manager) => {
      await manager
        .getRepository(Correction)
        .update(correction.id, { status: CorrectionStatus.UNDER_DTOE_REVIEW });
      const dtoeUsers = await this.dtoeUsers(manager);
      await this.notifyUsers(
        manager,
        dtoeUsers,
  ...
  private async toDetailDto(correction: Correction, user: User) {
    ...
    const myOpenRemarksCount =
      user.role === Role.CFO
        ? correction.remarks.filter(
            (r) => r.cfoId === user.cfoId && r.status !== RemarkStatus.CLOSED,
          ).length
        : 0;
  ...
  private async recomputeStatusAfterCfoAction(
    ...
    if (statuses.some((s) => s.status === CfoStatusValue.RETURNED)) {
      ...
      statuses.length > 0 &&
      statuses.every((s) => s.status === CfoStatusValue.APPROVED)
    ) {
